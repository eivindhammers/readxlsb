...
read_xlsb <- function(path, sheet = NULL, range = NULL, col_names = TRUE, col_types = NULL,
  ...
  # Normalize the target path by removing leading slash if present
  target_path <- sub("^/", "", xlsb_env$sheets[idx, "Target"])
  idx <- which(grepl(target_path, xlsb_contents[, "Name"], fixed = TRUE))
